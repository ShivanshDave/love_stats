...
smart_raw <- read_xpt("MMSA2017/MMSA2017.xpt") %>% tbl_df
```
...
```{r ingest_simpler}
smart_ohio_raw <- read_csv("data/smart_ohio_raw.csv") %>% tbl_df
...
### Summary of the `smart_ohio` file
...
### Inspecting the Cleveland-Elyria Data

```{r}
smart_cle %>%
    select(-SEQNO, -mmsa_code) %$%
    mosaic::inspect(.)
```
